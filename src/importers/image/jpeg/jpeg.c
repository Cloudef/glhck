...
static void fatal(j_common_ptr cinfo)
{
   (void)cinfo;
   struct jpegErrorStruct *errmgr = (struct jpegErrorStruct*)cinfo->err;
...
/* failure handler */
static void error2(j_common_ptr cinfo, int msg_level)
{
...
static int check(chckBuffer *buffer)
{
   ...
   struct jpegErrorStruct jerr;
   struct jpeg_decompress_struct cinfo;
   cinfo.err = jpeg_std_error(&(jerr.pub));
   jerr.pub.error_exit = fatal;
   jerr.pub.emit_message = error2;
   jerr.pub.output_message = error;
...
GLHCKAPI const char* glhckImporterRegister(struct glhckImporterInfo *info)
{
   ...
   RET(0, "%s", "glhck-importer-jpeg");
   return "glhck-importer-jpeg";
}
