...
// FIXME: TEMP
#include "render/OpenGLHelper.h"

/* \brief create new renderbuffer */
GLHCKAPI glhckRenderbuffer* glhckRenderbufferNew(int width, int height)
{
...
GLHCKAPI int glhckFramebufferAttachBuffer(glhckFramebuffer *object, glhckRenderbuffer *buffer,
      glhckTextureFormat storageFormat, glhckFramebufferAttachmentType attachment)
{
   glhckFramebuffer *old;
   ...
   glBindRenderbuffer(GL_RENDERBUFFER, buffer->object);
   glRenderbufferStorage(GL_RENDERBUFFER, glhTextureFormatForGlhckFormat(storageFormat), buffer->width, buffer->height);
   glFramebufferRenderbuffer(GL_FRAMEBUFFER, glhAttachmentTypeForGlhckType(attachment), GL_RENDERBUFFER, buffer->object);
