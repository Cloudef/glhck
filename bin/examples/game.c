...
typedef struct GameActor {
   ...
   glhckHandle object, muzzle;
   glhckHandle muzzleBone;
   glhckHandle animator;
...
typedef struct GameWindow {
   ...
   glhckHandle world, bulletObject, shadowObject;
   GameBullet *bullet;
...
{
   if (camera->handle) glhckHandleRelease(camera->handle);
   free(camera);
}
...
static void gameActorFree(GameWindow *window, GameActor *actor)
{
   ...
   if (actor->object) glhckHandleRelease(actor->object);
   if (actor->muzzle) glhckHandleRelease(actor->muzzle);
   if (actor->animator) glhckHandleRelease(actor->animator);
   free(actor);
}
...
#if 0
      glhckMaterialDiffuseu(glhckObjectGetMaterial(glhckObjectChildren(actor->object, NULL)[0]), r, g, b, 255);
#endif
   }
...
{
   if (text->handle) glhckHandleRelease(text->handle);
   free(text);
}
...
#endif
   if (window->bulletObject) glhckHandleRelease(window->bulletObject);
   if (window->shadowObject) glhckHandleRelease(window->shadowObject);
   if (window->world) glhckHandleRelease(window->world);
...
static GameWindow* gameWindowNew(int argc, char **argv)
{
   ...
   for (size_t i = 0; i < glhckRendererCount(); ++i) {
      ...
      if (glhckDisplayCreate(window->width, window->height, i))
         break;
   ...
   if (!(window->shadowObject = glhckSpriteNewFromFile("bin/media/area/shadow.png", 7.0f, 7.0f, NULL, NULL)))
      goto fail;

   if (!(window->world = glhckModelNew("bin/media/area/area.glhckm", 8.0f, NULL)))
      goto fail;
...
#if 0
   size_t numChilds;
   const glhckHandle *childs = glhckObjectChildren(window->world, &numChilds);
   for (size_t i = 0; i < numChilds; ++i)
...
static void renderAllCulled(glhckHandle object, glhckFrustum *frustum, unsigned int *culled)
{
   ...
   for (size_t i = 0; i < numChilds; ++i) renderAllCulled(childs[i], frustum, culled);
   glhckRenderObject(object);
}

static void gameWindowRender(GameWindow *window, float interpolation)
{
   kmVec3 intrp;
   kmVec3Intrp(&intrp, glhckCameraGetPosition(window->camera->handle), &window->camera->position, interpolation);
   glhckCameraPosition(window->camera->handle, &intrp);
   kmVec3Intrp(&intrp, glhckCameraGetTarget(window->camera->handle), &window->camera->target, interpolation);
   glhckCameraTarget(window->camera->handle, &intrp);
   ...
   glhckTextColoru(window->text->handle, 0, 0, 0, 255);
   ...
   glhckRenderText(window->text->handle);
   glhckTextClear(window->text->handle);

   glhckTextColoru(window->text->handle, 255, 255, 255, 255);
