...
{
   GLFWwindow* window;
   glhckHandle texture, material, cube, sprite, cube2, sprite3, camera;
   const kmAABB *aabb;
   ...
   for (size_t i = 0; i < glhckRendererCount(); ++i) {
      ...
      if (glhckDisplayCreate(WIDTH, HEIGHT, i))
         break;
...
#  define ASSIMP_PATH "example/media/player.x"
   ...
   } else if ((cube = glhckModelNewEx(ASSIMP_PATH, 0.1f, NULL, GLHCK_IDX_USHRT, GLHCK_VTX_V3S))) {
      glhckHandle mat, tex;
   ...
   glhckHandle plane = glhckPlaneNew(200.0f, 200.0f);
   ...
   glhckHandle text = glhckTextNew(512, 512);
   ...
   glhckHandle rttText = 0, rttText2 = 0;
   glhckTextColoru(text, 255, 255, 255, 255);
   ...
   float xspin = 0.0f;
   size_t numBones = 0;
   const glhckHandle *bones = glhckObjectBones(cube, &numBones);
   glhckHandle animator = 0;
   ...
   while (RUNNING && glfwGetKey(window, GLFW_KEY_ESCAPE) != GLFW_PRESS) {
      ...
      glhckObjectTarget(sprite3, glhckCameraGetPosition(camera));
      ...
      glhckTextColoru(text, 255, 0, 0, 255);
   ...
   /* should cleanup all objects as well */
   glhckTerminate();
   glfwTerminate();
   return EXIT_SUCCESS;
}
